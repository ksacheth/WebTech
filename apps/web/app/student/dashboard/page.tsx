...
import { buildStudentExamInstructionsPath } from "../exams/entry-access";
...
export default function LabProctorDashboard() {
  ...
  const upcomingExams = exams.filter(
    (exam) =>
      exam.isActive && new Date(exam.startTime).getTime() > currentTimeMs,
  );
        ...
        <header className="flex items-center justify-between border-b border-primary/10 bg-white px-6 md:px-10 py-4 sticky top-0 z-50 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-sm font-semibold text-red-600 transition-colors hover:bg-red-100"
              title="Logout"
            >
                ...
                <p className="text-slate-600">
                  {liveCount > 0 ? (
                    <>
                      ...
                      {upcomingCount > 0 &&
                        `and ${upcomingCount} upcoming session${upcomingCount !== 1 ? "s" : ""} scheduled`}
                      .
                    </>
                  ) : upcomingCount > 0 ? (
                    <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          >
                            <div className="p-6 md:flex gap-6">
                              ...
                              <div className="md:w-2/3 flex flex-col justify-between">
                                <div>
                                  {latestAttempt ? (
                                    <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-[11px] font-semibold text-slate-600">
                                      <span className="material-symbols-outlined text-sm">
                                        flag
                                      </span>
                                      Attempt status: {latestAttempt.status}
                                    </div>
                                  ) : null}
                    ...
                    {upcomingCount === 0 ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {upcomingExams.slice(0, 4).map((exam) => (
                          <div
                            key={exam.id}
                            className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm hover:border-secondary transition-colors"
                          >
                            ...
                            <h5 className="font-bold text-primary mb-1 truncate">
                              {exam.title}
                            </h5>
                ...
                <div className="space-y-8">
                  ...
                  <section className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    {exams.filter((e) => e.attempts && e.attempts.length > 0)
                      .length === 0 ? (
                            ...
                            return (
                              <div key={exam.id}>
                                <div className="flex items-center justify-between group cursor-pointer">
                                  <div>
                                    <p className="text-sm font-bold text-primary group-hover:underline truncate max-w-35">
                                      {exam.title}
                                    </p>
                                    <p className="text-[10px] text-slate-500 italic">
                                      {attempt.completedAt
                                        ? `Submitted ${new Date(attempt.completedAt).toLocaleDateString("en-IN", { month: "short", day: "numeric" })}`
                                        : "In progress"}
                                    </p>
                                  </div>
                                  <div className="text-right">
                                    ...
                                    <p
                                      className={`text-[10px] font-bold ${attempt.status === "COMPLETED" ? "text-green-500" : "text-yellow-500"}`}
                                    >
                                      {attempt.status}
                                    </p>
                                  </div>
